...
class SimDataRangeClassifyRD(data.Dataset):
    """
    ...
    def __init__(self, split, transform=None, squeeze=False, sensors=None):
        ...
        super(SimDataRangeClassifyRD, self).__init__()
...
class SimDataRangeClassifyRI(data.Dataset):
    """
    Dataset class for simulated data classification and ranging tasks. This dataloader is stochastic in that it, with 50-50 chance, chooses to 
    load noise or the signal. If it loads the signal it randomly chooses a TOSSIT to load the signal from. If it loads noise, the signal is
    randomly rotated.
    ...
    def __init__(self, split, transform=None, sensors=None):
        ...
        super(SimDataRangeClassifyRI, self).__init__()

        # make sure split is valid
        assert split in ['train', 'test', 'val'], "'split' must be one of 'train', 'test', 'val'"

        # get data file
        self.data, self.labels, self.noise = self._load_h5()
    ...
    def __getitem__(self, idx):
        ...
        if np.random.choice([0, 1]):
            inputs = self.data[[idx]]
            target_c = self._from_numpy(np.asarray([1]))
            target_r = self._from_numpy(self.labels[[idx],-3]) / self.max_r
        else:
            inputs = self.noise[[np.random.choice(self.noise_inds)]]
            inputs = inputs[[0],:(config['signal']['fs'] * config['signal']['T'])]
            inputs = random_wrap(inputs)
            target_c = self._from_numpy(np.asarray([0]))
            target_r = self._from_numpy(np.asarray([-1]))
    ...
    def _load_h5(self):
        file = h5py.File(config['dataset']['data_directory'] + "/VDS_main.h5", 'r', libver='latest')
        file_n = h5py.File(config['dataset']['data_directory'] + "/" + config['dataset']['noise_data'], 'r', libver='latest')
        return file['data'], file['labels'], file_n['data']

###########################################################
#                Dataloader Constructors                  #
###########################################################

def get_dataloaders_range_classify_RD(splits, batch_size, shuffle=True, drop_last=False, transform=None, squeeze=False, num_workers=10, pin_memory=False):
    """
    ...
    # prepare datasets
    datasets = {x : SimDataRangeClassifyRD(split=x, transform=data_transform[x], squeeze=squeeze) for x in data_transform.keys()}
