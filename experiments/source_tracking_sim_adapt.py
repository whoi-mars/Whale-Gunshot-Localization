...
def run_simulation(source_params, std_list, TOSSIT_locations, seed, localizer_params, loc0=None, bearing0=None, beamwidth=None):
    ...
    for i, std in enumerate(std_list):
        ...
        for step, (range_measurements, source_assocaitions, TOSSIT_associations, source_locs, source_ids) in enumerate(zip(range_measurements_list, source_associations_list, TOSSIT_associations_list, source_locs_list, source_ids_list)):
            ...
            assocs_est, locs_est = L.adaptive_localize(range_measurements)
            successful = (len(assocs_est) > 0)

            if successful and (not possible):
                results["FP"] = True
            elif (not successful) and possible:
                results["FN"] = True
...
if __name__ == "__main__":
    ...
    ############ 
    seed = 1324
    csv_path = os.path.join(PROJECT_ROOT_DIR, "experiments", "results", "tracking_sim", "source_tracking_sim_results.csv")
    stats_csv_path = os.path.join(PROJECT_ROOT_DIR, "experiments", "results", "tracking_sim", "source_tracking_sim_stats.csv")
    plot_path = os.path.join(PROJECT_ROOT_DIR, "experiments","results", "tracking_sim")
    source_params = [(1, 200), (50, 200), (70, 170), (85, 135)] # source_params = [(1,160), (40, 160), (60, 140), (85, 125)]
    ...
    if args.save_figs:
        ...
        fig5, spax = plt.subplots(1, max_sources, figsize=(15,10), sharey=True)
        ...
        fig6, spax = plt.subplots(1, max_sources, figsize=(15,10), sharey=True)
        ...
        df_high = df[(df["std"] == 660) & (df["num_sources"] == max_sources) & (df["FP"] == False) & (df["FN"] == False) & (df["over_predict_sources"] == False)]
        fig7 = plt.figure()
        ax = fig7.gca()
        counts, bins = np.histogram(df_high['percent_possible_detections']*max_sources, bins=np.arange(0,max_sources+1)+0.5)
        ax.hist(bins[:-1], bins, weights=counts, ec='k')
        ...
        fig8 = plt.figure()
        ax = fig8.gca()
        bins = np.arange(0,max_sources+1)+0.5
        counts = []
        for i in range(max_sources):
            counts.append(100*np.sum(~np.isnan(df_high[f'loc_error_{i}'])) / len(df_high[f'loc_error_{i}']))
        ax.hist(bins[:-1], bins, weights=counts, ec='k')
        ax.xaxis.set_major_locator(MaxNLocator(integer=True))
        ax.grid(axis='y')
        ax.set_axisbelow(True)
