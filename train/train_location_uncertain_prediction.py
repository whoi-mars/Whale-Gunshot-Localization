...
from whale_gunshot_localization.datasets.dataloaders import get_dataloaders
from whale_gunshot_localization.utils.transformations import get_image_transform
from whale_gunshot_localization.models.tcn_archs import FusionTCN
from whale_gunshot_localization.losses.localization_losses import UncertaintyPredictionLocLoss
...
if not args.no_wb:
    ...
    if args.wb_id is not None:
        api = wandb.Api()
        run = api.run("markg98/gunshot-localization-uncertain/" + args.wb_id)
    ...
    # intialize
    wandb.init(
        project="gunshot-localization-uncertain",
        config={
...
# set loss
criterion = UncertaintyPredictionLocLoss()
    ...
    for epoch in range(start_epoch, end_epoch+1):
        ...
        for phase in ['train', 'val']:
            ...
            for step, (inputs, x_targets, y_targets) in enumerate(tqdm(dataloaders[phase], disable=not verbose)):
                ...
                y_mse = F.mse_loss(outputs[:,[2]].detach() * max_y / 1000, y_targets * max_y / 1000, reduction='none')
