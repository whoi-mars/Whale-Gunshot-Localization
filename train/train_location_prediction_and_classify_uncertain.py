...
from whale_gunshot_localization.models.tcn_archs import TCNRangeAndClassifyUncertain
from whale_gunshot_localization.losses.multitask_losses import UncertainSelectiveMSEAndClassHeter
...
if not args.no_wb:
    ...
    if args.wb_id is not None:
        api = wandb.Api()
        run = api.run("markg98/simultaneous-range-classify-uncertain/" + args.wb_id)
    ...
    # intialize
    wandb.init(
        project="simultaneous-range-classify-uncertain",
        config={
...
model = TCNRangeAndClassifyUncertain(input_size=dl['train'].dataset.size[0], 
                                     num_channels=args.channels,
                                     kernel_size=args.kernel_size,
                                     dropout=args.dropout).to(device)

# set loss
criterion = UncertainSelectiveMSEAndClassHeter()
        ...
        for phase in ['train', 'val']:
            ...
            for step, (inputs, targets_c, targets_r) in enumerate(tqdm(dataloaders[phase], disable=not verbose)):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if phase == 'train':
                        if args.clip > 0:
                            torch.nn.utils.clip_grad_norm_([p for p in model.parameters()], args.clip)
                        loss.backward()
                        optimizer.step()
                ...
                outputs_r_TP = outputs[torch.where((F.softmax(outputs[:,2:4],dim=1)[:,1].squeeze() >= 0.5) & (targets_c.squeeze() == 1))[0],0].detach()
                targets_r_TP = targets_r[torch.where((F.softmax(outputs[:,2:4],dim=1)[:,1].squeeze() >= 0.5) & (targets_c.squeeze() == 1))].detach()
                ...
                running_corrects += ((F.softmax(outputs[:,2:4].detach(),dim=1)[:,1].squeeze() >= 0.5) == targets_c.detach().squeeze()).sum()
            ...
            print("{} Loss: {:.4f} -- RMSE: {:.4f} km -- ACC: {:.4F}".format(phase, epoch_loss, torch.sqrt(epoch_r_mse), running_corrects / running_example_count))
