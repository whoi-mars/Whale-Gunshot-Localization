...
# get dataloaders
dl = get_dataloaders_range_classify_RI(splits=['train', 'val'],
                                       batch_size=args.batch_size,
                                       drop_last=False,
                                       shuffle=True,
    ...
    for epoch in range(start_epoch, end_epoch+1):
        ...
        for phase in ['train', 'val']:
            ...
            for step, (inputs, targets_c, targets_r) in enumerate(tqdm(dataloaders[phase], disable=not verbose)):
                ...
                outputs_r_TP = outputs[torch.where((F.softmax(outputs[:,1:],dim=1)[:,1].squeeze() >= 0.5) & (targets_c.squeeze() == 1))[0],0:1].detach()
                targets_r_TP = targets_r[torch.where((F.softmax(outputs[:,1:],dim=1)[:,1].squeeze() >= 0.5) & (targets_c.squeeze() == 1))[0]].detach()
                
                # make sure true positives exist
                if len(outputs_r_TP) > 0 and len(targets_c) > 0:
                    r_mse = F.mse_loss(outputs_r_TP.squeeze() * max_r / 1000, targets_r_TP.squeeze() * max_r / 1000, reduction='none')
