"""
Script to plot and compare acoustically derived whale locations and visual estimate densities.
"""
...
if __name__ == "__main__":
    ...
    # limit dataframes to date range
    df = df[(args.start <= df["bin"]) & (df["bin"] <= args.end)]
    df_vis = df_vis[(args.start <= df_vis["bin"]) & (df_vis["bin"] <= args.end)]

    ########################################
    #     plot all acoustic detections     #
    ########################################
    ...
    for _, dfg in id_grouped:
        ...
        TOSSIT_IDs.append([config["TOSSIT"]["ids"].index(str(dfg.loc[i, "sensor"])) for i in range(len(dfg))])
    locs_est = np.stack([y, x], axis=1)

    plotting.plot_spatial_density(locs_est, TOSSIT_ids=TOSSIT_IDs, title="Acoustic Detection Density Map", ax=axs, bins=args.bins, buffer=args.buffer, norm=True, bathym=False, sensors=True, GDOP_norm=True)

    fig.savefig(os.path.join(fig_dir, "spatial_density.png"))

    #############################################################
    # plot intersection of acoustic/visual detections together  #
    #############################################################
    ...
    plotting.plot_spatial_density(locs_est, title="Acoustic Detection Density Map", ax=axs[0], bins=args.bins, buffer=args.buffer, norm=True, bathym=True, sensors=True)

    lon, lat = [], []
    for _, row in df_vis.iterrows():
        lat += [row['LATITUDE']] * row['NUMBER']
        lon += [row['LONGITUDE']] * row['NUMBER']
    locs_vis = np.stack([lat, lon], axis=1)

    plotting.plot_spatial_density(locs_vis, title="Visual Detection Density Map", ax=axs[1], bins=args.bins, est_latlon=True, buffer=args.buffer, norm=True, bathym=True, sensors=True)
    fig.tight_layout()

    # save figure
    fig.savefig(os.path.join(fig_dir, "spatial_density_comparison.png"))
